...
def shepardsInterpolation(data,mask,xx,yy,p):
    ...
    def calcIDWValue(data,ind,xx,yy,p):
        ...
        det=(((xx - xx[ind]) ** 2 + (yy - yy[ind]) ** 2))
        det2=np.copy(det)
        det3=r-det2

        #det2[i,j]=[max(0, det3[i,j]) for i in range(det.shape[0]) for j in range(det.shape[1])]
        det2=np.maximum(det3,0)
        den=r*det
        #print("done")
        weight=(det2/den)**p
        #print("done")
    ...
    for ind in (indices):
        data[ind[0], ind[1]] = calcIDWValue(data, ind, xx, yy, p)
    print("done")
    return data
...
def saveSatelliteData(temp1,temp2,file,alg,dataName,folderName):
    ...
    originalfigname = (os.path.join(str(alg)+str(dataName)+"_"+str(file) + 'Original.png'))
    preditedfigname = (os.path.join(str(alg)+str(dataName)+"_"+str(file) + 'Predicted.png'))
