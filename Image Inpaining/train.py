...
def multigpu_graph_def(model, data, config, gpu_id=0, loss_type='g'):
    with tf.device('/cpu:0'):
        imagesAll = data.data_pipeline(config.BATCH_SIZE)
        images=imagesAll[0]
        images1=imagesAll[1]
    if gpu_id == 0 and loss_type == 'g':
        _, _, losses = model.build_graph_with_losses(
            images,images1, config, summary=True, reuse=True)
    else:
        _, _, losses = model.build_graph_with_losses(
            images,images1, config, reuse=True)
    if loss_type == 'g':
...
if __name__ == "__main__":
    ...
    for i in range(len(fnames)):
        tempStr = (str(fnames[i])).split(',')
        fnames[i] = tempStr[0], tempStr[1]

    data = ng.data.DataFromFNames(
        fnames, [config.IMG_SHAPES,config.IMG_SHAPES], random_crop=config.RANDOM_CROP)
    #print(data)
    imageAll = data.data_pipeline(config.BATCH_SIZE)
    images=imageAll[0]
    images1=imageAll[1]
    #print("batch image shape",images)
    #print("batch image shape",np.shape(imageAll))
    # main model
    model = InpaintCAModel()
    g_vars, d_vars, losses = model.build_graph_with_losses(
        images,images1, config=config)
    # validation images
    if config.VAL:
    ...
    if not config.PRETRAIN_COARSE_NETWORK:
        print('pretrain coarse netwok true')
