...
from inpaint_ops import random_bbox, bbox2mask, local_patch,rowColMask,checkerboardMask
...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x,x2, mask, config=None, reuse=False,
    ...
    def build_graph_with_losses(self, batch_data,batch_data2, config, training=True,
                                summary=False, reuse=False):
        batch_pos = batch_data / 127.5 - 1.
        batch_pos2 = batch_data2 / 127.5 - 1.
        # generate mask, 1 represents masked point
        bbox = random_bbox(config)
        print("####build_graph_with_losses########")
        #mask = bbox2mask(bbox, config, name='mask_c')
        #mask=rowColMask(bbox,config,name='mask_c')
        mask=checkerboardMask(bbox,config,name='mask_c')
        print("first generated mask shape",mask.shape)
        ###first generated mask shape (1, 256, 256, 1)
        
        batch_incomplete = batch_pos*(1.-mask)
        batch_2 = batch_pos2
        #####batch_incomplete shape (2, 256, 256, 3)
        print("\nbatch_incomplete shape",batch_incomplete.shape)
        x1, x2, offset_flow = self.build_inpaint_net(
            batch_incomplete,batch_2, mask, config, reuse=reuse,training=training,padding=config.PADDING)
    ...
    def build_server_graph(self, batch_data, reuse=False, is_training=False):
        ...
        """
        # generate mask, 1 represents masked point
        batch_raw,batch_raw2, masks_raw = tf.split(batch_data, 3, axis=2)
        
        masks = tf.cast(masks_raw[0:1, :, :, 0:1] > 127.5, tf.float32)

        batch_pos = batch_raw / 127.5 - 1.
        batch_pos2 = batch_raw2 / 127.5 - 1.
        batch_incomplete = batch_pos * (1. - masks)
        batch2=batch_pos2
        # inpaint
        x1, x2, flow = self.build_inpaint_net(
            batch_incomplete,batch2, masks, reuse=reuse, training=is_training,
            config=None)
        batch_predict = x2
        batch_predict1 = x1
        # apply mask and reconstruct
        batch_complete = batch_predict*masks + batch_incomplete*(1-masks)
        batch_complete1=batch_predict1*masks + batch_incomplete*(1-masks)
        return batch_complete,batch_complete1
