...
data_dir = "training_ImageData_SMOIS/**_low.png"
# data_dir = "Dataset/**.png"
epochs = 1400
batch_size =64
...
generator.compile(loss=vgg_loss,optimizer=common_optimizer)
#ensorboard.set_model(discriminator)
gloss = []
for epoch in range(epochs):
    ...
    print(np.min(low_resolution_images))
    print(np.max(low_resolution_images))
    ...
    print(np.min(low_resolution_images))
    print(np.max(low_resolution_images))
    g_loss = generator.train_on_batch(low_resolution_images,high_resolution_images)



    print('epoch: %d, [ Generator :: loss: %f]' % (epoch, g_loss))
...
fig = plt.figure()
plt.plot(gloss, 'b--')
plt.savefig('ganVGGloss_smoisMask.png')
plt.close()
...
generator.save("initialModel_VGGloss_smoisMask.h5")
...
for epoch in range(epochs):
    ...
    print("beforetraining", generated_high_resolution_images.shape)
    ...
    if epoch % 10 == 0:
        ...
        for index, img in enumerate(generated_images):
            save_images(low_resolution_images[index], high_resolution_images[index], img,
                        path="testLogs/img_{}_{}".format(epoch, index))
...
generator.save("genmodelTest.h5")
discriminator.save("dismodelTest.h5")
'''
